...
export default function SendXrayReport() { 
  ...
  const handleSubmit = async (values, { resetForm }) => {
    ...
    axios
      .post("https://api.desherkhobor24.com/customer/send-patient", formData, {
  ...
  const handleFileChange = async (event,setFieldValue) => {
    ...
    try {
      const response = await axios.post('http://23.227.167.208/upload/', formData);
      const responseData = response.data;
 
    // Store the image URLs in component state
    ...
    setFieldValue("imageurl", imageURLs);
      ...
      for (let index = 0; index < responseData.data.length; index++) {
        const src = responseData.data[index];
        queryHTML += (
          `<div class="image-list" id="parent_${index}">
      ...
      document.getElementById('preview').innerHTML = queryHTML;
  ...
  return (
    ...
    >
      ...
      <input
        ...
        id="file"
        className=""
        accept=".png, .jpg, .jpeg, .gif,.dcm,.bmp"
        required
        multiple 
